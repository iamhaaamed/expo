...
export const initSpeechService = async () => {
  let response = await fetch(
    endpoint + "speaker/verification/v2.0/text-independent/profiles",
    {
      method: "POST",
      headers: {
  ...
  if (json?.profileId) return json?.profileId;
...
export const PrepareRequirementForViceScan = async () => {
  ...
  try {
    ...
    if (!scanVoiceUrl) {
      ...
      let voicePath;
      if (user?.scanVoice) voicePath = await DownloadFile(user?.scanVoice);
